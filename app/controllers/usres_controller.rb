class UsresController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @usre.save
        ...
        format.json { render :show, status: :created, location: @usre }
      else
  ...
  def update
    respond_to do |format|
      if @usre.update(usre_params)
        ...
        format.json { render :show, status: :ok, location: @usre }
      else
  ...
  def destroy
    @usre.destroy
    respond_to do |format|
      format.html { redirect_to usres_url, notice: 'Usre was successfully destroyed.' }
